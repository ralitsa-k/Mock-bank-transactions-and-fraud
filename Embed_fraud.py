...
large_non_fraud_data = pd.read_csv(save_path + 'final_logical_non_fraud.csv')

# define how many to be scammed 
perc_scammes = 18/100
# get N scammed 
n_scammed = int(len(large_non_fraud_data.customer_id.unique())*perc_scammes)
scammed_ids = random.choices(large_non_fraud_data.customer_id.unique(),k=n_scammed)

# Get the data for scammed customers only 
scammed_sample_data = large_non_fraud_data[large_non_fraud_data['customer_id'].isin(scammed_ids)]

# Load the data with fraud cases with descriptions 
fraud_cases = pd.read_csv(source_d_path + 'fraud_with_descriptions.csv')
unique_cases = fraud_cases.case_id.unique()
transaction_values = fraud_cases.groupby('case_id')['transaction_value'].mean()
# all fraud transactions in quartiles 
transaction_quartiles = np.percentile(transaction_values, [25, 50, 75])
# transaction in the upper quartiles 
high_value_transaction = fraud_cases.loc[fraud_cases.case_value > transaction_quartiles[2],:]

# assign high fraud transaction values to high-paid individuals (scammed individuals)
# first get salaries and obtain the quartiles of salaries of scammed customers 
salaries = scammed_sample_data[scammed_sample_data.type == 'income'].groupby('customer_id')['Amount'].mean().sort_values()
quartiles = np.percentile(salaries, [25, 50, 90])

# match_names of columns of fraud data and non-fraud data 
fraud_cases = fraud_cases.rename(columns = {'transaction_value':'Amount',
                             'transaction_description': 'Descriptions',
                             'category':'Category',
                             })

#%% Clean the fraud data and add frad transactions to selected customers 

# data without CEO scam 
no_ceo = fraud_cases.loc[fraud_cases.fraud_type!='CEO',:]
...
romance_only = no_ceo.loc[no_ceo.fraud_type=='Romance']
no_romance = no_ceo.loc[no_ceo.fraud_type!='Romance']
    
# get number of months in non-fraud data 
n_of_months = len(scammed_sample_data.loc[:, 'month'].unique())
...
fraud_and_no = pd.DataFrame()
for cust in scammed_sample_data.customer_id.unique():
    # get the data per customer 
    curr_data = scammed_sample_data.loc[scammed_sample_data['customer_id'] == cust, :].copy()
    # add column fraud type 
    curr_data.loc[:, 'fraud_type'] = 'none'
    
    # If the income of this person is in a high quartile, assign Romance to them 
    # If not, assign other types of scam 
    if curr_data.loc[curr_data.Category == 'Income', 'Amount'].iloc[1] > quartiles[2]:
       # nn is exact number of fraud transactions to be added  
       nn = random.randint(romance_per_month-2, romance_per_month)
       # these are some random rates to be used for the fraud transactions 
       sample_dates = curr_data.loc[:, ['date', 'month', 'customer_id']].sample(nn).reset_index(drop=True)
       fraud_d = romance_only.sample(n=nn).reset_index(drop=True)
    else:
       nn = random.randint(noromance_per_month-2, noromance_per_month)
       sample_dates = curr_data.loc[:, ['date', 'month', 'customer_id']].sample(nn).reset_index(drop=True)
       fraud_d = no_romance.sample(n=nn).reset_index(drop=True)
