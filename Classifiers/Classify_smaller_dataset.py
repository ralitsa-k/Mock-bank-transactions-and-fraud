...
df_sel = df.loc[:,['date','Amount','is_scam_transaction']]
le= LabelEncoder()
...
# Features
X_features = df.loc[:, ['Category', 'Amount', 'date']]
...
X_categ_train = vectorizer.fit_transform(X_train['Category'])
...
X_features_train = pd.concat([X_categ_X_train, X_amount_train, X_dates_train], axis=1)
X_features_test = pd.concat([X_categ_X_test, X_amount_test, X_dates_test], axis=1)
...
predictions_trained = classifier.predict(X_features_train)


# Write predictions into the original dataframe 
y_test['preds'] = predictions
df_out = pd.merge(df,y_test[['preds']],how = 'left',left_index = True, right_index = True)
df_out.to_csv('PredictionsResults/original_data_with_predicted_values.csv')
...
df2 = pd.read_csv('OutputData/data_with_classified_scam_almost_final_Interest_fixed.csv')
df2 = df.loc[~df2.Category.isnull(), :]
# PREDICTOR
...
X_categ_X_test = pd.DataFrame(X_categ_test.toarray(), columns=vectorizer.get_feature_names_out())
# Concatenate all the DataFrames
X_test = pd.concat([X_categ_X_test, X_amount_test, X_dates_test], axis=1)
